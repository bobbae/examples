...
struct ucb {
    ...
    int finished, sock, fd, port, sockbufsz, verbose, ipgfixed, sbufsz, rbufsz, eof, quiet;
    unsigned int elapsedusecs,  okpkts, ipg, secs, numcached,
        numfree, invalid, duplicates, seq, lastack, iter,
        chunksz, wchunk, curchunk, poolsz, badpkts, rate, desiredrate,
        bmsz, mtu, filenamesz, acked, plsz, pktsz, lastseq ;
};
...
#define UCMAXTRACE          10
    ...
    if (err > 0) fprintf(stderr, "errorstr: %s", strerror(err));
    va_start(ap, msg);
...
void *ucalloc(size_t sz)
{
    ...
    if (xp == NULL) ucfatal("ucalloc");
    return xp;
}

void ucfree(void *xp)
{
    if (xp == NULL) ucfatal("ucfree null");
    free(xp);
}
...
int ucsetsockbufs()
{
    ...
    if (ucb->sockbufsz == 0) ucb->sockbufsz = 1024 * 512;
    ucb->sbufsz = ucb->sockbufsz;
    if (setsockopt(ucb->sock, SOL_SOCKET, SO_SNDBUF, &ucb->sbufsz, sizeof(ucb->sbufsz)) < 0) 
        ucfatal("setsockopt");
    ucb->sbufsz = 0;     sz = sizeof(ucb->sbufsz);
    if (getsockopt(ucb->sock, SOL_SOCKET, SO_SNDBUF, &ucb->sbufsz, &sz) < 0) ucfatal("getsockopt");
    ucb->rbufsz = ucb->sockbufsz;
    if (setsockopt(ucb->sock, SOL_SOCKET, SO_RCVBUF, &ucb->rbufsz, sizeof(ucb->rbufsz)) < 0) 
        ucfatal("setsockopt");
    ucb->rbufsz = 0;     sz = sizeof(ucb->rbufsz);
    if (getsockopt(ucb->sock, SOL_SOCKET, SO_RCVBUF, &ucb->rbufsz, &sz) < 0) ucfatal("getsockopt");
    return 0;
}
...
int uctestbit(unsigned int seq)
{
    ...
    bit = seq & 7;
    byte = seq >> 3;
    if ((ucb->bitmap[byte] >> bit) & 1) return 1;
    return 0;
}
...
void ucclearbits()
{
    ...
    if (ucb->verbose)  ucprintln("clear all bits");
}

void ucallocbitmap()
{
    ucb->bmsz = MAX((ucb->chunksz)>>3, ucb->plsz);
    if (ucb->verbose) ucprintln("bmsz %d", ucb->bmsz);
    ucb->bitmap = (unsigned char *)ucalloc((size_t)ucb->bmsz);
    if (ucb->verbose)  ucprintln("allocated bitmap");
}

void ucallocrcache()
{
    ucb->rcache = (struct ucbuf **)ucalloc((size_t)ucb->chunksz  * sizeof(struct ucbuf *));
    if (ucb->verbose)  ucprintln("allocated rcache");
}
...
struct ucbuf *ucallocbuf()
{
    ...
    buf = (struct ucbuf *)ucalloc((size_t) sizeof(struct ucbuf) + ucb->pktsz);
    buf->flag = UCFREE;
...
void ucallocpool()
{
    ...
    if (ucb->verbose)  ucprintln("allocated buffer pool");
}
...
struct ucwcache *ucallocwcache()
{
    ...
    cache->count = 0;
    cache->next = NULL;
    cache->wcache = (struct ucbuf **)ucalloc((size_t)ucb->chunksz  * sizeof(struct ucbuf *));
    if (ucb->verbose)  ucprintln("alloc new wcache");
    return cache;    
}
...
void ucfreewcache(struct ucwcache *cache)
{
    ...
    ucfree(cache->wcache);
    cache->wcache = NULL;
    if (ucb->verbose)  ucprintln("free wcache");
    ucfree(cache);
}
...
void ucpushwcachelist(struct ucwcache *cache)
{
    ...
    if (ucb->verbose) ucprintln("push new wcache");
    ucassert(cache != NULL);
    ucassert(cache->next == NULL);
    ucassert(cache->wcache != NULL);
...
struct ucwcache *ucpopwcachelist()
{
    ...
    if (ucb->verbose) ucprintln("pop new wcache");
    return pp;
}

void ucinsertwcache(struct ucbuf *buf, unsigned int seq, struct ucbuf **wcache, unsigned int chunk)
{
    struct ucpkthdr *pkt = NULL;
    
    if (ucb->verbose) ucprintln("cached seq %d", seq);
    pthread_mutex_lock(&ucb->cachemutex);
    if (!buf) ucfatal("null buf");
    buf->flag = UCCACHED;
    pkt = buf->pkt;
    ucb->numcached++;
    seq = seq - (chunk * ucb->chunksz);
    if (wcache[seq] != NULL) ucfatal("chunk %d cached slot %d not empty", chunk, seq);
    wcache[seq] = buf;
    pthread_mutex_unlock(&ucb->cachemutex);
}
...
void ucinsertrcache(struct ucbuf *buf, unsigned int seq)
{
    ...
    if (ucb->verbose) ucprintln("cached seq %d", oseq);
    pthread_mutex_lock(&ucb->cachemutex);
    if (!buf) ucfatal("null buf");
    buf->flag = UCCACHED;
    pkt = buf->pkt;
    ucb->numcached++;
    seq = seq - (ucb->curchunk * ucb->chunksz);
    if (ucb->rcache[seq] != NULL) ucfatal("cached slot %d not empty", seq);
    ucb->rcache[seq] = buf;
    pthread_mutex_unlock(&ucb->cachemutex);
}
...
void ucfreebuf(struct ucbuf *buf, struct ucbuf **wcache, unsigned int chunk)
{
    ...
    if (!buf) ucfatal("putfree: null buf");
    if (buf->pkt == NULL) ucfatal("putfree: null pkt inside buf");
...
void ucrecyclercache()
{
    ...
    if (ucb->verbose) ucprintln("freeall");
    pthread_mutex_lock(&ucb->bufmutex);
    ...
    pthread_mutex_unlock(&ucb->bufmutex);
    if (ucb->verbose)  ucprintln("recycle rcache");
}
...
struct ucbuf *ucgetrcache(unsigned int seq)
{
    ...
    if (tmpbuf->flag != UCCACHED) ucfatal("seq %d (chunk %d seq %d) not in the cache", oseq, ucb->curchunk, seq);
    ucb->numcached--;
    pthread_mutex_unlock(&ucb->cachemutex);
    return tmpbuf;
}

struct ucbuf *ucgetwcache(unsigned int seq, struct ucbuf **wcache, unsigned int chunk)
{
    struct ucbuf *tmpbuf = NULL;
    ...
    if (tmpbuf->flag != UCCACHED) ucfatal("not cached");
    ucb->numcached--;
    pthread_mutex_unlock(&ucb->cachemutex);
    return tmpbuf;
}
...
void ucprintinfo()
{
    ucprintln("peer IP %s port %d", inet_ntoa(ucb->saddr.sin_addr), ucb->port);
    ucprintln("plsz %d chunksz %d filename %s", ucb->plsz, ucb->chunksz, ucb->filename);
...
int ucopenfile()
{
    ...
    if (ucb->filename[0] == '-' && ucb->filename[1] == '\0') return (fd = 0);
...
int uccreatefile()
{
    ...
    if (ucb->filename[0] == '-' && ucb->filename[1] == '\0') return (fd = 1);
    if ((fd = open(ucb->filename, O_WRONLY|O_TRUNC|O_CREAT, 0644)) < 0) ucfatal("createfile");
    return fd;
}
...
struct ucpkthdr *ucreadfile(unsigned int seq)
{
    ...
    if (fd < 0) ucfatal("bad file descriptor");
    buf = ucgetrcache(seq);
    if (buf) return buf->pkt;
    if (ucb->eof) ucfatal("eof and no entry in cache for seq %d", seq);
    buf = ucgetbuf();
    pkt = buf->pkt;
    if ((len = read(fd, (char *)pkt + sizeof(struct ucpkthdr), ucb->plsz)) < 0)
        ucfatal("read error, size %d", ucb->plsz);
    pkt->len = htons(len3 = len);
    if (len < ucb->plsz) {
        if (ucb->verbose) ucprintln("short read %d < %d", len, ucb->plsz);
        len2 = len;
        while (len2 < ucb->plsz) {
            if ((len = read(fd, (char *)pkt + sizeof(struct ucpkthdr) + len2, ucb->plsz - len2)) < 0)
                ucfatal("read error 2, size", ucb->plsz - len2);
            if (ucb->verbose) ucprintln("extra read %d", len);
            if (len == 0) {
                pkt->flag = UCLASTFLAG;
                ucb->eof++;
                ucb->lastseq = seq;
                if (ucb->verbose) ucprintln("last detected at seq %d len %d", seq, len);
                break;
            } else if (len == ucb->plsz - len2) {
                pkt->len = htons(len3 = len + len2);
                pkt->flag = 0;
                if (ucb->verbose) ucprintln("extra read successful");
                break;
            } else {
                if (ucb->verbose) ucprintln("extra read still short: %d < %d", len + len2, ucb->plsz - len2);
            }
            len2 = len2 + len;
        }
    } else {
...
void ucrequestbitmap()
{
    ...
    if (ret <= 0)    ucfatal("send");
    if (ret != sizeof(struct ucpkthdr))   ucfatal("send");
    if (ucb->verbose) ucprintln("requestbitmap");
}
...
void ucsendbitmap()
{
    ...
    if (ret <= 0)     ucfatal("writev");
    if (ret != sizeof(struct ucpkthdr) + len)   ucfatal("writev");
    if (ucb->verbose) ucprintln("sendbitmap ack %d", ucb->okpkts);
}
...
void ucreport()
{
    if (ucb->mode == 't')
        ucprintln("[%d] seq %d ack %d tot %d ipg %d cur %d", 
                  ucb->secs, ucb->seq, ucb->acked, ucb->totalpkts, ucb->ipg, ucb->curchunk);
    else
        ucprintln("[%d] ok %d tot %d dup %d cached %d free %d cur %d wchunk %d",
...
void *uctimer(void *param)
{
    while (!ucb->finished) {
        sleep(1);
        ++ucb->secs;
        if (!ucb->quiet) ucreport();
        if (ucb->mode == 'r' && ucb->okpkts > 0) ucsendbitmap();
    }
    return 0;
}
...
void ucwritefile(unsigned int len, void *buf)
{
    ...
    if (ret <= 0)  ucfatal("write error, size %d, ret %d", len, ret);
    ucb->xferbytes += ret;
}
...
void *ucfilewriter(void *param)
{
    ...
    while ((wc = ucpopwcachelist()) == NULL) {
        if (ucb->verbose) ucprintln("null cache list wait...");
        usleep(100000);
    }
    wcache = wc->wcache;
    for (;;) {
        ...
        pkt = buf->pkt;
        if (seq != pkt->pktnum) ucfatal("filewriter: wchunk %d, seq %d != pktnum %d", ucb->wchunk, seq, pkt->pktnum);
        if (ucb->verbose) ucprintln("writing pkt seq %d sz %d", pkt->pktnum, ntohs(pkt->len));
        ucwritefile(ntohs(pkt->len), (void *)((char *)pkt + sizeof(struct ucpkthdr)));
        if (ucb->lastseq == seq) {
            if (ucb->verbose)  ucprintln("written last pkt, seq %d", ucb->lastseq);
            break;
        }
        ucfreebuf(buf, wcache, ucb->wchunk);
        if ((++seq % ucb->chunksz) == 0) {
            if (ucb->verbose) ucprintln("free wc chunk %d", ucb->wchunk);
            ucfreewcache(wc);
            while ((wc = ucpopwcachelist()) == NULL) {
                if (ucb->verbose) ucprintln("null cache list wait...");
                usleep(100000);
    ...
    if (ucb->verbose) ucprintln("exiting writer");
    return 0;
}
...
void ucrecvfile()
{
    ...
    if (ucb->sock < 0) ucfatal("socket");
    ucb->saddr.sin_family = AF_INET;
    ucb->saddr.sin_port = htons(ucb->port);
    ucb->saddr.sin_addr.s_addr = INADDR_ANY;
    if (ucsetsockbufs(ucb->sockbufsz) < 0) ucfatal("ucsetsockbufs");
    if (bind(ucb->sock, (struct sockaddr *)&ucb->saddr, sizeof(ucb->saddr)) < 0) ucfatal("bind");
    pkt = (struct ucpkthdr *) ucalloc((size_t)(sizeof(struct ucpkthdr) + ucb->plsz));
    ipkt = (struct ucpkthdr *) ucalloc((size_t)(sizeof(struct ucpkthdr) + ucb->plsz));
    initpl = (struct ucbnitpl *)((char*)pkt + sizeof(struct ucpkthdr));
    for (;;) {
        saddrlen = sizeof(ucb->saddr);
	ret = recvfrom(ucb->sock, (void*)pkt, ucb->pktsz, 0, (struct sockaddr *) &ucb->saddr, &saddrlen);
	if (ret < 0 && errno != EINTR) ucfatal("recvfrom");
        if (ret != ucb->pktsz) ucfatal("recvfrom");
        if (pkt->type != UCBNIT) {
            ucb->invalid++;
            continue;
        }
        ...
        if (ucb->filename == NULL) {
            ...
            ucb->filename = (char*) ucalloc((size_t)(ucb->filenamesz+1));
        ...
        if (getcwd(ucb->workdir,(size_t)sz) == NULL) ucfatal("getcwd");
        if (!ucb->quiet) ucprintinfo();
        for (delay = 0; delay < 20; delay++) {
            ret = sendto(ucb->sock, (void*)pkt, ucb->pktsz, 0, (struct sockaddr*) &ucb->saddr, sizeof(ucb->saddr));
            if (ret <= 0) ucfatal("sendto");
            if (ret != ucb->pktsz) ucfatal("sendto");
            usleep(delay);
        }
        break;
    }
    ...
    if (connect(ucb->sock, (struct sockaddr*)&ucb->saddr, sizeof(ucb->saddr)) < 0) ucfatal("connect");
    ucb->fd = uccreatefile();
    ucallocbitmap();
    ucallocpool();
    ...
    while (!ucb->finished) {
        if (buf == NULL) buf = ucgetbuf();
        ipkt = buf->pkt;
        ret = recv(ucb->sock, (void *)ipkt, ucb->pktsz, 0);
	if (ret < 0 && errno != EINTR) ucfatal("recv");
        if (ret < nread) ucfatal("recv");
        switch (ipkt->type) {
        case UCBITMAP:
            if (ucb->verbose) ucprintln("bitmap request");
            ucsendbitmap();
            break;
        case UCFINISHED:
            ucb->finished++;
            if (ucb->verbose) ucprintln("finished msg received");
            break;
        case UCDATA:
            ucb->totalpkts++;
            ucb->totalbytes += ret;
            ...
            if (ipkt->flag & UCLASTFLAG) {
                ucb->lastseq = seq;
                chunklim = seq + 1;
                if (ucb->verbose)  ucprintln("received lastseq %d chunklim %d", ucb->lastseq, chunklim);
            }
            if (seq < (ucb->curchunk * ucb->chunksz) || seq >= chunklim){
                if (ucb->verbose) ucprintln("chunk %d range (%d ~ %d) bad seq %d", chunk, chunk * ucb->chunksz, chunklim, seq);
                ucb->badpkts++;
                break;
            }
            if (ucsetbit(seq)) {
                ++ucb->duplicates;
                if (ucb->verbose) ucprintln("duplicate seq %d", seq);
                break;
            }
            if (ucb->verbose) ucprintln("ok seq %d", seq);
            ucb->okbytes += ntohs(ipkt->len);
            ucinsertwcache(buf, ipkt->pktnum, wcache, chunk);
            if ((++ucb->okpkts % ucb->chunksz) == 0) {
                if (ucb->verbose) ucprintln("chunk %d -> %d", chunk, chunk + 1);
                ucsendbitmap();
                ucb->curchunk = ++chunk;
                wc = ucallocwcache();
                ucpushwcachelist(wc);
            ...
            if ((ucb->okpkts % 100) == 0) ucsendbitmap(); /* XXX: */
            if (ucb->okpkts == ucb->lastseq + 1) {
                if (ucb->verbose) ucprintln("finished, okpkts %d", ucb->okpkts);
                ucb->finished++;
            }
            buf = NULL;
            break;
    ...
    if (ucb->verbose) ucprintln("finished, out of mainloop");
    ucb->finished++;
    pkt->type = UCFINISHED;
    pkt->pktnum = htonl(ucb->okpkts);
    if (ucb->verbose) ucprintln("sending FINISHED msg");
    ...
    if (ucb->verbose) ucprintln("exiting receiver");
}
...
void ucsendblock(unsigned int seq)
{
    ...
    ucdelayusec(ucb->ipg);
    if (ucb->verbose && (pkt->flag & UCLASTFLAG)) ucprintln("sending LAST msg seq %d", ntohl(pkt->pktnum));
    ret = send(ucb->sock, (void*)pkt, ucb->pktsz, 0);
    if (errno == ECONNREFUSED)      return;
    if (ret <= 0) ucfatal("send");
    if (ret != ucb->pktsz) ucfatal("send");
    ucb->totalbytes += ret;
    ucb->totalpkts++;
    gettimeofday(&tv, 0);
    diff = ucusecdiff(&ucb->starttime, &tv);
    ucb->rate = (unsigned int) ((float)ucb->totalbytes * 1000000/ diff);
    if (ucb->ipgfixed) return;
    if (ucb->desiredrate == 0) return;
    if (ucb->rate > ucb->desiredrate)  ucb->ipg++;
    else if (ucb->ipg > 0)  ucb->ipg-- ;
}
...
void ucgetinput(struct ucpkthdr *ipkt, int nread)
{
    ...
    if (ret < 0 && errno != EINTR) ucfatal("recv");
    if (ret < sizeof(struct ucpkthdr)) ucfatal("recv");
    switch (ipkt->type) {
    case UCFINISHED:
        ret = recv(ucb->sock, (void *)ipkt, nread, 0);
	if (ret < 0 && errno != EINTR) ucfatal("recv");
        if (ucb->verbose) ucprintln("finished, received FINISHED msg.");
        ucb->finished++;
    ...
    case UCBITMAP:
        ...
        if (ret <= 0) ucfatal("readv");
        if (ret < len + sizeof(struct ucpkthdr)) ucfatal("readv");
        break;
    default:
        ret = recv(ucb->sock, (void *)ipkt, nread, 0);
	if (ret < 0 && errno != EINTR) ucfatal("recv");
...
void ucxmitfile() 
{
    ...
    for (delay = 0; delay < 1000; delay++) {
        for (ix = 0; ix < 20;ix++) {
            ...
            usleep(delay);
            nread = 0;
            if (ioctl(ucb->sock, FIONREAD, &nread) < 0)  ucfatal("ioctl");
            if (nread >= ucb->pktsz)  break;
        }
        if (nread < ucb->pktsz) continue;
        saddrlen = sizeof(rsaddr);
        ret = recvfrom(ucb->sock, (void *)ipkt, ucb->pktsz, 0, (struct sockaddr *)&rsaddr, &saddrlen);
	if (ret < 0 && errno != EINTR) ucfatal("recvfrom");
        if (ret != ucb->pktsz)     ucfatal("recvfrom");
        if (ipkt->type != UCBNIT) continue;
        if (memcmp((void*)pkt,(void*)ipkt, ucb->pktsz) == 0) {
            if (connect(ucb->sock,(struct sockaddr *)&rsaddr, sizeof(rsaddr)) < 0) ucfatal("connect");
            connected++;
            break;
        }
        fprintf(stderr,"error: remote init pkt echo does not match.\n");
        fflush(stderr);
        ucterminate(1);
    }
    if (!connected) {
    ...
    for (;;) {
        ...
        if (ucb->verbose) ucprintln("start sending chunk %d, range %d ~ %d", chunk, seq, (chunk + 1) *ucb->chunksz);
        chunkdone = 0;
        ucb->iter = 0;
        chunklim = seq + ucb->chunksz;
        while (!chunkdone) {
            nread = 0;
            if (ioctl(ucb->sock, FIONREAD, &nread) < 0) ucfatal("fionread");
            while (nread >= sizeof(struct ucpkthdr)) {
                ucgetinput(ipkt, nread);
                if (ipkt->type == UCFINISHED) {
                    ucb->acked = ntohs(ipkt->pktnum);
                    if (ucb->verbose) ucprintln("finished, received FINISHED");
                    break;
                }
                if (ipkt->type == UCBITMAP) {
                    if (ntohl(ipkt->pktnum) == (chunk + 1) * ucb->chunksz) {
                        if (ucb->verbose) ucprintln("chunk %d done", chunk);
                        chunkdone++;
                        break;
                    }
                }
            ...
            if (ucb->finished) {
                if (ucb->verbose) ucprintln("finished, break 1");
                break;
            }
            if (chunkdone) {
                if (ucb->verbose) ucprintln("chunk %d done", chunk);
            ...
            if (ucb->verbose) ucprintln("sending seq %d", seq);
            ucsendblock(seq);
            seq = ucadvseq(seq, chunklim);
            if (ucb->verbose)  ucprintln("next seq %d", seq);            
        }
        if (ucb->finished) {
            if (ucb->verbose) ucprintln("finished, break 2");
            break;
        }
        chunkdone = 0;
        ucb->curchunk = ++chunk;
        if (ucb->verbose) ucprintln("chunk %d -> %d", chunk - 1, chunk);
        ucrecyclercache();
        ucclearbits();
    }
    ucfree(pkt);
    ucfree(ipkt);
    if (ucb->verbose) ucprintln("exiting transmitter.");
}
...
int ucmain(int argc, char *argv[])
{
    ...
    if (ucb->filename == NULL)  ucb->filename = "testfile";
    pthread_attr_init(&ucb->attr);
    pthread_create(&ucb->timer, &ucb->attr,  uctimer, NULL);
    if (ucb->plsz == 0)  ucb->plsz = ucb->mtu - 20 - 8 - sizeof(struct ucpkthdr);
    ...
    if (!ucb->quiet) {
        time(&tm);
        ts = ctime(&tm);
        ucprintln("%s version %s , start time: %s process id %d", argv[0],
                  ucversion, ts, getpid());
    }
    if (ucb->mode == 't') {
        if (!ucb->quiet) ucprintln("Sending...");
        ucxmitfile();
    } else {
        if (!ucb->quiet) ucprintln("Receiving...");
